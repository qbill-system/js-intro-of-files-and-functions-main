...
function createProfileDescription(person) {
  ...
  */
  ...
  const likesToCode = person.likesToCode;

  if (likesToCode) {
    return `My name is ${name} and I like to code.`;
  } else {
    return `My name is ${name} and I don't like to code.`;
  }
}
...
function readTrafficLight(color) {
  ...
  color = color.toLowerCase(); // Convert to lowercase for case-insensitivity
  ...
  */
  switch (coin) {
    ...
    case "10p":
      ...
      break;
    default:
      // If the provided coin is not recognized, do nothing
      break;
  }
...
function addCoins(coinCollection, coin) {
  ...
  */
  switch (coin) {
    case "1p":      
      console.log(coinCollection[0]);
      coinCollection[0].push('1p');
      break;
    case "2p":
      coinCollection[1].push('2p');
      break;
    case "5p":
      coinCollection[2].push('5p');
      break;
    case "10p":
      coinCollection[3].push('10p');
      break;
    default:
      // Invalid coin type
